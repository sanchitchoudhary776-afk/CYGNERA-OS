...
function NewPathModal({ onClose, onSave }) {
  ...
  return (
    <div onClick={onClose} style={{ position:'fixed',inset:0,zIndex:100,display:'flex',alignItems:'flex-end',justifyContent:'center',background:'var(--overlay)',animation:'modalFadeIn 220ms ease both' }}>
      <div onClick={e=>e.stopPropagation()} style={{ width:'100%',maxWidth:500,maxHeight:'92dvh',display:'flex',flexDirection:'column',background:'var(--s2)',border:'1px solid var(--card-b-h)',borderRadius:'var(--r-xl) var(--r-xl) 0 0',boxShadow:'var(--modal-sh)',animation:'modalSlideUp 340ms var(--bounce) both' }}>
            ...
            <div style={{ display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:8 }}>
              {STYLES.map(st=>(
                <button key={st.id} onClick={()=>setForm(p=>({...p,style:st.id}))}
                  style={{ padding:'11px 8px',borderRadius:'var(--r-md)',border:`2px solid ${form.style===st.id?'var(--p)':'rgba(255,255,255,0.06)'}`,background:form.style===st.id?'rgba(9,205,131,0.10)':'var(--s3)',cursor:'pointer',transition:'all 200ms var(--bounce)',transform:form.style===st.id?'scale(1.03)':'scale(1)',textAlign:'left' }}>
            ...
            <div style={{ display:'flex',gap:7 }}>
              {HOURS.map(h=>(
                <button key={h} onClick={()=>setForm(p=>({...p,hours:h}))}
                  style={{ flex:1,padding:'9px 4px',borderRadius:'var(--r-md)',border:`1px solid ${form.hours===h?'var(--p)':'rgba(255,255,255,0.07)'}`,background:form.hours===h?'rgba(9,205,131,0.10)':'transparent',color:form.hours===h?'var(--p)':'var(--t4)',cursor:'pointer',fontSize:12,fontWeight:700,transition:'all 150ms ease' }}>
                  {h}h
                </button>
              ))}
            </div>
        ...
        <div style={{ display:'flex',gap:10,padding:'14px 20px',borderTop:'1px solid rgba(9,205,131,0.07)',paddingBottom:'max(14px,env(safe-area-inset-bottom))' }}>
          ...
          <button onClick={create} disabled={loading} className="btn btn-primary"
            style={{ flex:1,padding:'11px',opacity:loading?0.7:1,cursor:loading?'not-allowed':'pointer' }}>
    ...
    <div className="page">
      ...
      <div style={{ display:'flex',flexDirection:'column',gap:16 }}>
        <div className="card fadeup" style={{ padding:24,background:`linear-gradient(160deg,${color}0e,var(--s2))`,borderColor:`${color}20` }}>
          <h2 style={{ fontSize:'clamp(1.15rem,3vw,1.55rem)',fontWeight:800,color:'var(--t1)',letterSpacing:'-0.025em',marginBottom:6 }}>{path.title}</h2>
    ...
    <div className="page">
      ...
      {completed.length > 0 && (
        <div style={{ marginBottom:24 }}>
          ...
          <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))',gap:14,opacity:0.7 }}>
      ...
      {showNew && <NewPathModal onClose={()=>setShowNew(false)} onSave={p=>{ A.path.add(p); }}/>}
    </div>
  );
}
